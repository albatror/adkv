...
void Overlay::RenderMenu()
{
	...
	if (ImGui::BeginTabBar(XorStr("Tab")))
	{
		if (ImGui::BeginTabItem(XorStr("Main")))
		{
			...
			ImGui::SliderFloat(XorStr("AssistMe FOV"), &assistme_fov, 1.0f, 1000.0f, "%.2f");
			ImGui::SliderFloat(XorStr("AssistMe Max Distance"), &assistme_max_dist, 10.0f * 40, 500.0f * 40, "%.2f");
			ImGui::SliderFloat(XorStr("AssistMe Smooth"), &assistme_smooth, 1.0f, 300.0f, "%.2f");
			ImGui::Checkbox(XorStr("AssistMe Auto Shoot"), &assistme_auto_shoot);
