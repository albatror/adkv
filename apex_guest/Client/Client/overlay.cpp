...
void Overlay::RenderMenu()
{
	...
	if (ImGui::BeginTabBar(XorStr("Tab")))
	{
		if (ImGui::BeginTabItem(XorStr("Main")))
		{
			...
			ImGui::Separator();
			ImGui::Checkbox(XorStr("AimAssist (LSHIFT)"), &aimassist);
			ImGui::SliderFloat(XorStr("AimAssist FOV"), &aimassist_fov, 1.0f, 1000.0f, "%.2f");
			ImGui::SliderFloat(XorStr("AimAssist Max Distance"), &aimassist_max_dist, 10.0f * 40, 500.0f * 40, "%.2f");
			ImGui::SliderFloat(XorStr("AimAssist Smooth"), &aimassist_smooth, 1.0f, 100.0f, "%.2f");
			ImGui::Checkbox(XorStr("AimAssist Auto Shoot"), &aimassist_auto_shoot);
		...
		if (ImGui::BeginTabItem(XorStr("Visuals")))
		{
			...
			ImGui::Checkbox(XorStr("AimAssist Circle fov"), &v.aimassist_fov_circle);
			ImGui::SameLine();
...
DWORD Overlay::CreateOverlay()
{
	...
	while (running)
	{
		...
		if (v.aimassist_fov_circle)
		{
			ImGui::Begin("##aimassistcirclefov", nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar);
			auto draw = ImGui::GetBackgroundDrawList();
			draw->AddCircle(ImVec2(getWidth() / 2, getHeight() / 2), aimassist_fov, IM_COL32(0, 0, 255, 255), 100, 1.0f);
			ImGui::End();
		}
