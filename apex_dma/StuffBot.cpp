...
void StuffBotLoop()
{
    ...
    while (stuff_t)
    {
        ...
        // AimAssist logic
        if (aimassist && aimassist_aiming && aimentity != 0)
        {
            ...
            if (Target.isAlive() && (!Target.isKnocked() || firing_range) && is_aimentity_visible)
            {
                ...
                if (dist <= aimassist_max_dist)
                {
                    ...
                    {
                        QAngle aim_angles = CalculateBestBoneAim(LPlayer, aimentity, aimassist_fov, aimassist_smooth);
                        if (aim_angles.x != 0 || aim_angles.y != 0)
                        {
                            // Deadzone check to eliminate shaking
